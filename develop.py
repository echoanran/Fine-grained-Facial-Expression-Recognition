...
def main(args,
    ...
    if is_reload:
        os.makedirs(os.path.join(savefolder, 'kde'), exist_ok=True)
    ...
    while True:
        ...
        exp_final = np.argmax(
            np.sum(exp_preds_cum[max(0, frame_idx - 1 - 250):frame_idx], 0))
    ...
    print('\nEnd processing: {} frames in total, {} frames detected.'.format(
